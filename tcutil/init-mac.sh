...
if [ ! -f "$HOME/Library/Application Support/TorCash/torcash.conf" ]; then
    echo "Creating torcash.conf"
    mkdir -p "$HOME/Library/Application Support/TorCash/"
    echo "rpcuser=zcashrpc" > ~/Library/Application\ Support/TorCash/torcash.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/TorCash/torcash.conf"
    echo "Complete!"
fi
